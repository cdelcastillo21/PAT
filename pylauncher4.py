...
debugtracefile = open("debug.txt","w")
def DebugTraceMsg(msg, sw=False, prefix=""):
    ...
    if debugtracefile is not None:
        ...
        os.fsync(debugtracefile.fileno())
...
class Task:
    ...
    def start_on_nodes(self,**kwargs):
        ...
        # CED - Adding post process functionality 
        wrapped_post = self.post_process
        wrapped_cmd = re.sub("PYL_ID", str(self.taskid), self.command)
        if self.post_process is None:
            # If no post process command - Attach completion command to main command
            wrapped_cmd = self.completion.attach(wrapped_cmd)  
        else:
            # If post process command - Attach completion command to post process command
            wrapped_post = self.completion.attach(self.post_process)
        DebugTraceMsg(
            "starting task %d of size %d on <<%s>>\nin cwd=<<%s>>\ncmd=<<%s>>\npre=<<%s>>\npost=<<%s>>" % \
                (self.taskid,self.size,str(self.pool),os.getcwd(),wrapped_cmd, self.pre_process, wrapped_post),
            self.debug,prefix="Task")
        commandexecutor = self.pool.pool.commandexecutor
        self.starttime = time.time()
        commandexecutor.execute(wrapped_cmd, pool=self.pool, pre_process=self.pre_process, post_process=wrapped_post)
    ...
    def __repr__(self):
        s = "Task %d, commandline: [%s], pool size %d, pre: [%s], post: [%s]" \
            % (self.taskid, self.command, self.size, self.pre_process, self.post_process)
        return s
